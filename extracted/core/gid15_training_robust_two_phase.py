"""
Complete code-cell extraction from notebook8572cef2e5.ipynb.
...
for split in ['train', 'val']:
    imgs  = list((BASE / 'img_dir' / split).iterdir())
    masks = list((BASE / 'ann_dir' / split).iterdir())
    print(f"\n{split}:")
    print(f"  Images : {len(imgs)}  — sample: {imgs[0].name}")
    print(f"  Masks  : {len(masks)} — sample: {masks[0].name}")
    print(f"  Format : {set(f.suffix for f in imgs)}")

# Check mask class values
sample_mask = list((BASE / 'ann_dir' / 'train').iterdir())[0]
mask_arr    = np.array(Image.open(sample_mask))
print(f"\nUnique classes in mask : {np.unique(mask_arr)}")
print(f"Mask shape             : {mask_arr.shape}")
print(f"Mask dtype             : {mask_arr.dtype}")

# Check image
sample_img = list((BASE / 'img_dir' / 'train').iterdir())[0]
...
# ============================================================================
# SOURCE NOTEBOOK CELL 3 | CODE CELL 4
# ============================================================================
from pathlib import Path

BASE = Path('/kaggle/input/datasets/sheheryarkhan00/gid15-yrb')

# Check mask naming pattern
masks = sorted((BASE / 'ann_dir' / 'train').iterdir())
imgs  = sorted((BASE / 'img_dir' / 'train').iterdir())
...
# Check if masks pair to images
print("\nChecking pairs:")
for img in imgs[:5]:
    stem = img.stem
    matched = [m for m in masks if stem in m.name]
    print(f"  {img.name} → {[m.name for m in matched]}")


# ============================================================================
# SOURCE NOTEBOOK CELL 4 | CODE CELL 5
# ============================================================================
# ════════════════════════════════════════════════════════════════════════════
...
# ============================================================================
# SOURCE NOTEBOOK CELL 5 | CODE CELL 6
# ============================================================================
# ════════════════════════════════════════════════════════════════════════════
# Cell 2/3 — Dataset, class weights, combined loss, mIoU metric
# ════════════════════════════════════════════════════════════════════════════

def load_mask(path):
    """Load a segmentation mask as a 2D index array (no luminance remap)."""
    m = np.array(Image.open(path))
    if m.ndim == 3:        # val .tif masks come as replicated 3-channel
        m = m[:, :, 0]
    return m
...
class GID15Dataset(Dataset):
    def __init__(self, split, patch_size, transform, crops_per_img):
        ...
        mask_ext = '.png' if split == 'train' else '.tif'

        img_dir, ann_dir = BASE/'img_dir'/split, BASE/'ann_dir'/split
        self.pairs = []
        for img_path in sorted(img_dir.glob('*.tif')):
            mp = ann_dir / f'{img_path.stem}_15label{mask_ext}'
            if mp.exists():
    ...
    def __getitem__(self, idx):
        ...
        # Reject near-empty crops: prefer a crop with >10% labeled pixels
        for _ in range(10):
...
# ── Augmentation: moderate (enough to regularize, not destroy signal) ─────────
...
# ── Class weights (log-smoothed inverse frequency — tames imbalance) ─────────
def compute_class_weights(pairs, subsample=8):
    ...
    w = 1.0 / np.log(1.02 + freq)     # ENet-style smoothing
    w[IGNORE_IDX] = 0.0
...
# ── mIoU via confusion matrix (proper segmentation metric) ───────────────────
...
def miou_from_confmat(cm):
    ...
    valid = (union > 0)
    valid[IGNORE_IDX] = False          # exclude unlabeled
...
# ============================================================================
# SOURCE NOTEBOOK CELL 6 | CODE CELL 7
# ============================================================================
# ════════════════════════════════════════════════════════════════════════════
...
# ============================================================================
# SOURCE NOTEBOOK CELL 7 | CODE CELL 8
# ============================================================================
