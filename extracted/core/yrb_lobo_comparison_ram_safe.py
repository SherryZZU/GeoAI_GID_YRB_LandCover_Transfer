"""
Complete code-cell extraction from yrb-lulc-lobo-comparison22.ipynb.
...
# ============================================================================
# ============================================================================
# LEAVE-ONE-BASIN-OUT: Prithvi-EO-2.0 (LoRA) vs GID ResNet — RAM-safe version
...
# ---- index patches by basin (FILE PATHS only, no arrays in RAM) ------------
...
def load_sb(sb):                       # load one basin on demand
    Xs, Ys = [], []
...
print(f"{len(all_npz)} paths -> {len(files)} unique | basins:", sizes,
      "| total:", sum(sizes.values()))

# ---- normalization stats from memory-mapped samples ------------------------
...
for sb in FOLD_ORDER:
    ...
    del Xtr,Ytr,Xv,Yv,Xte,Yte; import gc; gc.collect()
